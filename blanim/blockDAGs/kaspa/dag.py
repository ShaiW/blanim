...
structure where each block has one parent selected parent, with the possibility of more
parents, forming a dag. The system uses a shared configuration pattern where all blocks
reference DEFAULT_KASPA_CONFIG for consistent visual styling across the entire dag.
...
class BitcoinDAG:
    def __init__(self, scene: HUD2DScene, chain_config: KaspaConfig = DEFAULT_KASPA_CONFIG):
        self.scene = scene
    ...
    def add_block(
            self,
            parent: Optional[KaspaLogicalBlock] = None,
        ...
        #TODO change this to multiple possible parents
    ...
    # TODO not required for DAG, same with repositioning logic, need to rewrite all positioning for DAG
    ...
    def _calculate_position(self, parent: Optional[KaspaLogicalBlock]) -> tuple[float, float]:
    ...
    def _collect_parent_level_animations(self, parent: KaspaLogicalBlock, animations: List):
    ...
    def _calculate_chain_length(self, block: KaspaLogicalBlock) -> int:
    ...
    def generate_chain(self, num_blocks: int) -> List[KaspaLogicalBlock]:
    ...
    def _generate_block_name(self, parent: Optional[KaspaLogicalBlock]) -> str:
    ...
    @staticmethod
    def get_past_cone(block: KaspaLogicalBlock) -> List[KaspaLogicalBlock]:
    ...
    @staticmethod
    def get_future_cone(block: KaspaLogicalBlock) -> List[KaspaLogicalBlock]:
    ...
    def get_anticone(self, block: KaspaLogicalBlock) -> List[KaspaLogicalBlock]:
